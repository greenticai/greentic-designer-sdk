...
A sixth thing shares the name on release lines that carry the addon kind, but is
not authored here: the `resource-request` record (`min-memory-mb`,
`min-cpu-millis`) in the addon WIT contract is what an *addon workload* asks the
platform for, and has nothing to do with the guest's own `memoryLimitMB`.
...
contributes no tools — today `bundle`, `deploy`, `provider`, `wasm-component`
and `mcp`. That list is read from each kind's own template, so it follows the
templates rather than needing an edit here.
