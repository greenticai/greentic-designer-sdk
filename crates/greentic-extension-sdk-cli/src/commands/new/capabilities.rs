...
use greentic_extension_sdk_contract::{
    AgenticWorkerMetadata, CapabilityId, CapabilityRef, ExtensionKind, ToolCapability,
    describe::ApiGrant,
...
/// The wire spelling of an extension kind (`ProviderExtension`), read off the
/// contract's own serde representation.
///
/// Derived rather than taken from a helper method so this compiles unchanged on
/// both release lines — `ExtensionKind::wire_name()` exists only on the 1.2.x
/// line — and so it cannot drift from what the field actually carries.
fn wire_name(kind: ExtensionKind) -> Option<String> {
    serde_json::to_value(kind).ok()?.as_str().map(str::to_owned)
}
...
fn unknown_extension_kind_note(kind: &str) -> Option<String> {
    let known: Vec<String> = ExtensionKind::ALL
        .iter()
        .copied()
        .filter_map(wire_name)
        .collect();
    (!known.iter().any(|k| k == kind)).then(|| {
        format!(
...
mod tests {
    ...
    fn every_contract_kind_is_accepted_without_a_note() {
        for kind in ExtensionKind::ALL {
            let wire = wire_name(kind).expect("kind serializes to a string");
            assert!(
                unknown_extension_kind_note(&wire).is_none(),
                "{wire} should be known"
            );
        }
    }
