...
fn permissions_llm_roles_default_empty_and_roundtrip() {
    // Old describe.json without llm_roles still parses.
    ...
    assert_eq!(back["llm_roles"][0], "sorla_composer");
}
...
fn permissions_llm_roles_omitted_when_empty() {
    ...
    assert!(
        !s.contains("llm_roles"),
        "llm_roles must be omitted when empty; got: {s}"
    );
}

#[test]
fn permissions_llm_roles_multiple_roles() {
    let v = serde_json::json!({ "llm_roles": ["sorla_composer", "sorla_reviewer"] });
    let p: Permissions = serde_json::from_value(v).expect("parses");
