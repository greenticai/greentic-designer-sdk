...
//!
//! This module is intentionally contract-free (only `semver` + `serde`) so the
//! resolver can be consumed by hosts — such as the designer — whose dependency
//! graph caps `greentic-extension-sdk-contract` below the version the registry
//! crate requires. The contract-dependent helpers (`check_updates`, `upgrade`,
//! `ExtensionUpdate`) live in `greentic-extension-sdk-registry::update` and
//! re-export `resolve`/`UpdateStatus` from here.
